    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (profileRes.data) {
          ...
          const workerUpi = (((profileRes.data as Record<string, unknown>).upi_id ??
            (profileRes.data as Record<string, unknown>).upi) as string) || "";
          setUpi(workerUpi);
  ...
  return (
    <div>
                ...
                {payouts.map((payout) => (
                  <TableRow key={payout.id}>
                    <TableCell className="font-medium">
                      {payout.payout_number || payout.id.slice(0, 8)}
                    </TableCell>
                    <TableCell>{formatDate(payout.initiated_at)}</TableCell>
                    <TableCell className="capitalize">{payout.plan_name || "—"}</TableCell>
                    <TableCell>{payout.claim_type || "—"}</TableCell>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            ...
            {!editingUpi && (
              <Button variant="outline" size="sm" onClick={() => setEditingUpi(true)}>
      ...
      <Dialog open={!!selectedPayout} onOpenChange={() => setSelectedPayout(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-display">Payout Receipt</DialogTitle>
          </DialogHeader>
          {selectedPayout && (
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Payout ID</span>
                <span>{selectedPayout.payout_number || selectedPayout.id.slice(0, 8)}</span>
              </div>
              <Separator />
              <div className="flex justify-between">
                <span className="text-muted-foreground">Date</span>
                <span>{formatDate(selectedPayout.initiated_at)}</span>
              </div>
              <Separator />
              <div className="flex justify-between">
                <span className="text-muted-foreground">Plan</span>
                <span className="capitalize">{selectedPayout.plan_name || "—"}</span>
              </div>
              <Separator />
              <div className="flex justify-between">
                <span className="text-muted-foreground">Claim Type</span>
                <span>{selectedPayout.claim_type || "—"}</span>
              </div>
              <Separator />
              <div className="flex justify-between">
                <span className="text-muted-foreground">Amount</span>
                <span className="font-semibold">₹{Number(selectedPayout.amount).toLocaleString("en-IN")}</span>
              </div>
              <Separator />
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span>{selectedPayout.upi || upi || "—"}</span>
              </div>
