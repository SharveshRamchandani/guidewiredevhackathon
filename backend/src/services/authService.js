...
async function registerWorker({ name, phone, password, platform, zone_id, city_id, upi }) {
  ...
  const { rows } = await query(
    `INSERT INTO workers
       (name, phone, password_hash, platform, zone_id, city_id, upi,
        kyc_status, risk_level, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending', 'low', NOW(), NOW())
     RETURNING id, name, phone, platform, zone_id, city_id, upi, kyc_status, risk_level, created_at`,
    [name, phone, password_hash, platform, zone_id || null, city_id || null, upi || null]
  );
...
async function loginWorker({ phone, password }) {
  const { rows } = await query(
    `SELECT id, name, phone, password_hash, platform, zone_id, city_id, upi, kyc_status, risk_level
...
async function getWorkerProfile(workerId) {
  const { rows } = await query(
    `SELECT w.id, w.name, w.phone, w.platform, w.zone_id, w.city_id,
            w.upi, w.kyc_status, w.risk_level, w.is_active,
            w.weekly_earnings, w.notifications, w.created_at,
            z.name AS zone_name, c.name AS city_name
     FROM workers w
